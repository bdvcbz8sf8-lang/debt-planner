...
test('avalanche puts higher APR debt first in payoff order', async ({ page }) => {
  ...
  await first.getByLabel('Остаток').fill('500000')
  await second.getByLabel('Остаток').fill('100000')
...
test('snowball puts lower balance debt first in payoff order', async ({ page }) => {
  ...
  await first.getByLabel('Остаток').fill('800000')
  await second.getByLabel('Остаток').fill('120000')
  await first.getByLabel('APR').fill('12')
  await second.getByLabel('APR').fill('5')
  await first.getByLabel('Платёж', { exact: true }).fill('20000')
  await second.getByLabel('Платёж', { exact: true }).fill('5000')
...
test('includeInPlan=false keeps loan in list but removes it from strategy focus', async ({ page }) => {
  ...
  await first.getByLabel('Остаток').fill('100000')
...
test('loan details show portfolio plan schedule after calculation', async ({ page }) => {
  ...
  await expect(firstCard.locator('text=Расчёт по портфелю')).toBeVisible()
})
